...
public class InitialService
{
	...
	public String getRestTime(String groupId) {
		...
		jp.co.pds.PJ0002.dao.util.MsGroupUtil msGroupUtil = new jp.co.pds.PJ0002.dao.util.MsGroupUtil(Integer.valueOf(groupId));
	...
	/**
	 * ユーザID、年、月を指定して勤務時間DTOリストを取得する
	 * @param userId
	 * @return
	 */
	public List<WorkTimeDTO> getWorkTimeDTOs(String userId, String year, String month) {
		List<WorkTimeDTO> workTimeDTOs = new ArrayList<WorkTimeDTO>();

		// 空の当月分を作成
		MsDateUtil msDateUtil = new MsDateUtil();
		List<MsDate> msDates = msDateUtil.getMsDates(Integer.valueOf(year), Integer.valueOf(month));
		for (MsDate msDate : msDates) {
			WorkTimeDTO workTimeDTO = new WorkTimeDTO();
			workTimeDTO.setCalendarDD(String.valueOf(msDate.getCalendarDd()));
			workTimeDTO.setWeekdayName(WeekdayCdEnum.getByCode(msDate.getWeekdayCd()).getShortName());
			workTimeDTO.setHolidayCd(msDate.getHolidayCd());
			workTimeDTOs.add(workTimeDTO);
		}

		// 勤務時間データに登録済みのデータがあればマージする
		DtWorkTimeUtil dtWorkTimeUtil = new DtWorkTimeUtil();
		List<DtWorkTime> dtWorkTimes = dtWorkTimeUtil.getDtWorkTimesByUserIdYyMm(userId, Integer.valueOf(year), Integer.valueOf(month));
		for (DtWorkTime dtWorkTime : dtWorkTimes) {
			int day = dtWorkTime.getCalendarDd();
			WorkTimeCopy.copy(dtWorkTime, workTimeDTOs.get(day - 1));
		}
